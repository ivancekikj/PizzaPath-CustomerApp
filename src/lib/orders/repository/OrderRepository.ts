...
async function getCurrentUserOrder(): Promise<Order | null> {
	const response = await axios.get(`${ApiData.ADMIN_APP_URL}/api/orders/current-order/`)
	if (response.data === '') {
		return null
	}
	...
	return {
		...
		items: response.data.items.map(
			(item: {
				...
				quantity: number
				topping_ids: Array<number>
				are_coupons_used: boolean
			}) => {
				return new OrderItem(
					item.id,
					{
						...
						toppings: item.food.toppings.map(
							(topping) =>
								({
									id: topping.id,
									name: topping.name,
...
async function deleteItem(id: number): Promise<void> {
	await axios.delete(`${ApiData.ADMIN_APP_URL}/api/orders/current-order/items/${id}/`, {
		headers: { 'Content-Type': 'application/json' }
	})
}

async function deleteCurrentUserOrder() {
	await axios.delete(`${ApiData.ADMIN_APP_URL}/api/orders/current-order/`, {
		headers: { 'Content-Type': 'application/json' }
	})
}
