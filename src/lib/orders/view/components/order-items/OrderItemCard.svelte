<script lang="ts">
	...
	async function onItemQuantityChange(): Promise<void> {
		let selectedPortionItems: OrderItem[] = $OrderStore!.items.filter(
			(i) => i.selectedPortionId === item.selectedPortionId && i.areCouponsUsed
		)
		const portion: FoodPortion = item.getSelectedPortion()
		let usedCoupons: number = selectedPortionItems
		...
		<div class="col-md-8 p-2">
			<div class="card-body d-flex flex-column justify-content-between h-100">
				<div class="d-flex justify-content-between align-items-center">
					...
					<button
						class="btn btn-primary red-button rounded-circle"
						on:click={deleteItem}
						aria-label="Delete"
						disabled={$OrderStore && $OrderStore.status !== 'edit'}
					>
				...
				<div class="row justify-content-between">
					<div class={item.food.toppings.length > 0 ? 'col-3' : 'col-5'}>
						<input
							...
							id="quantity"
							min="1"
							bind:value={item.selectedQuantity}
							on:change={onItemQuantityChange}
							disabled={$OrderStore && $OrderStore.status !== 'edit'}
						/>
					</div>
					<div class={item.food.toppings.length > 0 ? 'col-5' : 'col-7'}>
						<select
							class="form-select"
							id="size"
							bind:value={item.selectedPortionId}
							on:change={onItemPortionChange}
							disabled={$OrderStore && $OrderStore.status !== 'edit'}
						>
					...
					{#if item.food.toppings.length > 0}
						<div class="col-4">
							<button
								class="btn btn-primary green-button w-100"
								data-bs-toggle="modal"
								data-bs-target="#edit-toppings-modal"
								on:click={() => setItemForToppings(item)}>Toppings</button
							>
						</div>
					{/if}
				</div>
				<div class="d-flex justify-content-between">
					<div>
						<input
							...
							id="coupon"
							bind:checked={item.areCouponsUsed}
							on:change={onItemSelectionChange}
							disabled={($OrderStore && $OrderStore.status !== 'edit') ||
								!$EnabledItemCouponsStore.has(item.id)}
						/>
						<label for="coupon" class="form-label">Redeem coupons</label>
					</div>
					<div>
						Discount: <span class="fw-bold">{item.getSelectedPortion().discount * 100}%</span>
					</div>
