...
export const load: PageLoad = async () => {
	...
	const categoryCoupons = Array.from(couponsByCategoryId.values()).sort((a, b) =>
		a.categoryName.localeCompare(b.categoryName)
	)

	const postsTotalCount = await NewsletterPostsRepository.getCurrentUserReceivedPostsCount()
	let posts: NewsletterPost[] = []
	if (postsTotalCount > 0) posts = await NewsletterPostsRepository.getCurrentUserReceivedPosts(1)
